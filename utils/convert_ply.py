...
def convert_ply_to_obj(ply_fanme:str) -> None:
    ...
    obj_fname = os.path.join(base_dir, os.path.basename(ply_fanme).replace(".ply", ".obj"))
    mesh = Mesh(filename=ply_fanme)
    print(f"psbody.mesh.Mesh loaded from {ply_fanme}, vshape:{mesh.v.shape}, fshape:{mesh.f.shape}")
