...
class Faceformer(nn.Module):
    def __init__(self, n_verts: int, n_onehot: int):
        ...
        self.period = 60
        self.vertice_dim = n_verts
        self.audio_preprocessor = Wav2Vec2Processor.from_pretrained(
        ...
        # motion encoder
        self.vertice_map = nn.Linear(self.vertice_dim, self.feature_dim)
        # periodic positional encoding
        self.PPE = PeriodicPositionalEncoding(self.feature_dim, period=self.period)
        # temporal bias
        ...
        # style embedding
        self.obj_vector = nn.Linear(self.n_onehot, self.feature_dim, bias=False)
    ...
    def forward(self, audio, one_hot, template, **kwargs):
        self.device = audio.device
        frame_num = audio.shape[1] * 60 // 16000
        audio = self.audio_preprocessor(
            audio.squeeze(0), return_tensors="pt", sampling_rate=16000
        ).input_values.to(self.device)
        # tgt_mask: :math:`(T, T)`.
        # memory_mask: :math:`(T, S)`.
        template = template.unsqueeze(1).view(1, 1, -1)  # (1,1, V*3)
        ...
        return vertice_out.view(1, frame_num, -1, 3)

    def predict(self, audio, one_hot, template, **kwargs):
        return self(audio, one_hot, template, **kwargs)
        # frame_num = audio.shape[1] * 60 // 16000
        # audio = self.audio_preprocessor(audio, return_tensors="pt").input_values
        # template = template.unsqueeze(1).view(1, 1, -1)
...
if __name__ == "__main__":
    ...
    audio = torch.randn(1, 82400)  # 309
    hidden_states = audio_encoder(audio, "vocaset", frame_num=309).last_hidden_state
